...
- 📊 **Or glance at it.** A local dashboard lays every install out in a sortable table: what it is, which projects use it, installed vs. latest, and a freshness badge — with checkboxes to update the stale ones (or "Update all behind" in one click). Copies of the same skill collapse into one row, source-less skills tuck into a collapsed section with a **Set source…** action to adopt them (or **Mark local** to hush the ones that are yours), edited or unverified copies wear a muted amber badge instead of a false "stale", ignored skills and repos tuck into a collapsed **Ignored** section, and one button rescans.
...
| 🧩 **Adopted GitHub skill** | Once you've run `spur adopt`, its installed version vs. the repo's latest release tag — or, if there's no version recorded, whether your copy's files still match that tag. Copies you've edited show as **modified** and are never clobbered — updates refuse by default, and `--force` backs up first. |
